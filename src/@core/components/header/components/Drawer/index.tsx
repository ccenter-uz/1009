    ...
    <Drawer size={'xs'} isOpen={isOpen} placement='left' onClose={onClose}>
      ...
      >
        ...
        <DrawerBody>
                      ...
                      {link.subMenu.map(menu => (
                        <List key={menu.id}>
                          <Link href={menu.href} className={`${pathname == `/${locale}${menu.href}` ? 'active' : ''}`}>
                            <ListItem fontSize={scssVariables.fonts.paragraph} p={'0.3rem 0'} textAlign={'left'} borderBottom={'1px solid grey'}>
                              {menu.title}
                            </ListItem>
                          </Link>
                        </List>
          ...
          <Heading fontSize={scssVariables.fonts.paragraph} fontWeight={400} textAlign={'center'} mt={'3em'} mb={'20px'}>
            +1 (7635) 547-12-97
          </Heading>
          {colorMode === 'light' ? (
            <Box display={'flex'} justifyContent={'center'} alignItems={'center'} gap={'11px'}>
              ...
              />
              <Box fontSize={scssVariables.fonts.paragraph} color={scssVariables.mainColor}>Uz</Box>
            </Box>
          ) : (
            <Box display={'flex'} justifyContent={'center'} alignItems={'center'} gap={'11px'}>
              <Img
