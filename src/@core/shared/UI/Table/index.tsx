'use client'
import { ItableType } from '@/@core/shared/types/types'
    ...
    <>
      <TableContainer borderRadius={'4px'} border={border ? '1px solid #E2E8F0' : 'none'}>
        <Table aria-label='table'>
          <Thead>
            <Tr style={colorMode === 'dark' ? { background: '#484a4a' } : { background: ColBg || 'whitesmoke' }}>
              {columns.map(col => (
                <Th
                  fontWeight={600}
          ...
          <Tbody>
            {dataSource.map((row, rowIndex) => (
              <Tr
                _even={colorMode === 'dark' ? { background: '#484a4a' } : { background: RowBg || 'whitesmoke' }}
                key={rowIndex}
              >
