...
import { getUserForOperator } from './serverAction'

const UsersForOperator: FC = () => {
        ...
        return (
          <Box display={'flex'} alignItems={'center'} justifyContent={'center'}>
            <Tooltip label='Посмотреть' aria-label='tooltip'>
              <img
                onClick={() => router.push(`/operator/users/${record.id}`)}
                role='button'
                width={'18px'}
                height={'18px'}
  ...
  const getData = async (search: string | null, page: number | string, limit: number | string) => {
    ...
    //   pageSize: limit
    // }
    // const res = await getUserForOperator(query)
...
export default UsersForOperator
